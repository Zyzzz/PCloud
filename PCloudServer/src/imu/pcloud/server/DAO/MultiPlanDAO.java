...
/**
 	* A data access object (DAO) providing persistence and search support for MultiPlan entities.
 			* Transaction control of the save(), update() and delete() operations 
		can directly support Spring container-managed transactions or they can be augmented	to handle user-managed Spring transactions. 
		Each of these methods provides additional information for how to configure it for the desired type of transaction control. 	
	 * @see imu.pcloud.server.DAO.MultiPlan
  * @author MyEclipse Persistence Tools 
 */
public class MultiPlanDAO extends BaseHibernateDAO  {
	     private static final Logger log = LoggerFactory.getLogger(MultiPlanDAO.class);
		//property constants
    ...
    public MultiPlan findById( java.lang.Integer id) {
        ...
        try {
            MultiPlan instance = (MultiPlan) getSession()
                    .get("imu.pcloud.server.been.MultiPlan", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public MultiPlan merge(MultiPlan detachedInstance) {
        ...
        try {
            MultiPlan result = (MultiPlan) getSession()
